...
const BackTestsPage = () => {
  ...
  const {filter, reports, showDetails, showStart} = useAppSelector(state => state.backTests);
          ...
          <div className={styles.content_container}>
            <div className={styles.content_header}>
              <div className={styles.sorting_container}>
                <p>сортировка: </p>
                <Dropdown options={STATS_FIELDS.map(current => current)}
                          selected={orderBy} onChange={handleSetOrderBy} />
              </div>
              <PageSelector current={currentPage} total={reports.totalPages} setCurrent={handleSetCurrentPage} />
              <ActiveButton onClick={handleShowStart}>Запустить тест</ActiveButton>
            </div>

            <div className={styles.report_container}>
              {reports.reports.map((report, index) => (
                  <ReportItem data={report} onClick={handleShowDetails} key={index} />
              ))}
            </div>
          </div>
        </div>
