...
import {fetchCheck, fetchDeleteChatId, fetchSetChatId} from "../../slices/userSlice.ts";
...
const ProfilePage = () => {
  ...
  const handleApply = () => {
    dispatch(fetchSetChatId(id))
        .then(() => dispatch(fetchCheck()));
  }

  const handleDelete = () => {
    dispatch(fetchDeleteChatId())
        .then(() => dispatch(fetchCheck()));
  }
        ...
        <div className={styles.container}>
          <div className={styles.content_container}>
            ...
            <div>
              ...
              <p
                  className={
                    user.profit >= 0 ? styles.green : styles.red
                  }
              >{getProfit(user.profit)}</p>
            </div>
          </div>
        </div>
      </>
