...
const FilterWall = () => {
  ...
  const [strategyName, setStrategyName] = useState<string>(localFilter.strategyName);
  const [selectedInstrumentIds, setSelectedInstrumentIds] = useState<number[]>(localFilter.symbols);

  const [selectedTimeframes, setSelectedTimeframes] = useState<string[]>(localFilter.timeframes);
  ...
  const handleChangeStrategyName = (strategyName: string) => {
    ...
    setLocalFilter({...localFilter, strategyName: strategyName as StrategyNames});
  }
  ...
  return (
      <div className={styles.container}>
        <div>
          <h3>Стратегия</h3>
          <Dropdown options={STRATEGY_NAMES.map(current => current)}
                    selected={strategyName} onChange={handleChangeStrategyName}/>
        </div>

        <div>
          <h3>Инструмент</h3>
          <InstrumentCheckboxGroup instruments={items} selectedIds={selectedInstrumentIds}
                                   onChange={handleChangeSelectedInstrumentIds}/>
        </div>

        <div>
          <h3>Временные отрезки</h3>
          <CheckboxGroup options={timeframes} selected={selectedTimeframes}
